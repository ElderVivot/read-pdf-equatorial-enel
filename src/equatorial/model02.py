    ...
    async def processAsync(self):
        ...
        try:
            for idx, line in enumerate(self.__dataTxt):
                ...
                dataValueBeforeLine = returnDataInDictOrArray(self.__dataTxt, [idx - 1, 'dataLine'])
                try:
                    ...
                    if dataValue.find('MONOFASICO') >= 0 and tipoFornecimento == '':
                        tipoFornecimento = 'MONOFASICO'

                    if dataValue.find('BIFASICO') >= 0 and tipoFornecimento == '':
                        tipoFornecimento = 'BIFASICO'

                    if dataValue.find('TRIFASICO') >= 0 and tipoFornecimento == '':
                        tipoFornecimento = 'TRIFASICO'

                    if dataValue.find('NOTA FISCAL N') >= 0 and bairro == '':
                        positionNotaFiscal = dataValue.find('NOTA FISCAL')
                        bairro = treatTextField(dataValue[:positionNotaFiscal])
                        UCEnergia = bairro.split(' ')[-1]
                        bairro = ' '.join(bairro.split(' ')[:-1])
                        endereco = dataValueBeforeLine

                    if dataValue.find('CNPJ/CPF:') >= 0 and cnpj == '':
                        positionFind = dataValue.find('CNPJ/CPF:')
                        lenght = len('CNPJ/CPF:')
                        positionStart = positionFind + lenght
                        cnpj = treatNumberField(dataValue[positionStart:])[:14]

                    if dataValue.find('CEP:') >= 0 and cep == '':
                    ...
                    if dataValue.find('ENERGIA ATIVA FORNECIDA') >= 0:
                        ...
                        qtdKw = treatDecimalField(textEnergiaAtivaSplit[0])
                        amountInvoiceKw = treatDecimalField(textEnergiaAtivaSplit[2])
                    ...
                    if dataValue.find('VALOR MIN. FATURAVEL CUSTO DISP') >= 0:
                        ...
                        qtdKw = treatDecimalField(textValorMinFaturavelSplit[0])
                        amountInvoiceKw = treatDecimalField(textValorMinFaturavelSplit[2])

                    if dataValue.find('CONSUMO KWH + ICMS/PIS/COFINS') >= 0 and (qtdKw == 0 or amountInvoiceKw == 0):
                        # print(dataValue)
                        cadastradoPraReceberCredito = False
                        positionFind = dataValue.find('CONSUMO KWH + ICMS/PIS/COFINS')
                        lenght = len('CONSUMO KWH + ICMS/PIS/COFINS')
                        positionStart = positionFind + lenght
                        ...
                        elif textConsumoSplit[0] == 'KWH':
                            qtdKw = treatDecimalField(textConsumoSplit[1])
                            amountInvoiceKw = treatDecimalField(textConsumoSplit[3])
                        else:
                            qtdKw = treatDecimalField(textConsumoSplit[0])
                            amountInvoiceKw = treatDecimalField(textConsumoSplit[2])

                    if dataValue.find('CONSUMO') >= 0 and (qtdKw == 0 or amountInvoiceKw == 0) and dataValue.find('CONSUMO') < 5:
                        # print(dataValue)
                        cadastradoPraReceberCredito = False
                    ...
                    if dataValue.find('ENERGIA INJETADA') >= 0:
                        ...
                        qtdKwCredito = treatDecimalField(textEnergiaInjetadaSplit[0])
                        amountInvoiceKwCredito = treatDecimalField(textEnergiaInjetadaSplit[2])
                    ...
                    if dataValue.find('ILUMIN. PUBLICA - CIP') >= 0:
                        positionFind = dataValue.find('ILUMIN. PUBLICA - CIP')
                        lenght = len('ILUMIN. PUBLICA - CIP')
                        positionStart = positionFind + lenght
                        textContribPublic = treatTextField(dataValue[positionStart:])
                        textContribPublicSplit = textContribPublic.split(' ')
                        contribuicaoPublica = treatDecimalField(textContribPublicSplit[0])
